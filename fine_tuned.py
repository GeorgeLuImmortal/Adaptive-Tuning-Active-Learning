...
def train(train_dataset, model, tokenizer,loop,dataset_name,saving_steps,learning_rate):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args['gradient_accumulation_steps'] == 0:
                ...
                if saving_steps > 0 and global_step % saving_steps == 0:
                    ...
                    output_dir = os.path.join(args['output_dir'], str(loop),'%s-%s'%(dataset_name,str(global_step)))
...
def evaluate(model, tokenizer, prefix=""):
    ...
    eval_dataset = load_and_cache_examples(EVAL_TASK, tokenizer, evaluate=False)
    ...
    # for batch in tqdm_notebook(train_dataloader, desc="Evaluating"):
    #     model.eval()
    #     batch = tuple(t.to(device) for t in batch)
    ...
    #     logger.info("***** Eval results {} for training set *****".format(prefix))
...
def fine_tuned(loop,dataset_name,pre_trained,saving_steps,learning_rate):
    ...
    if args['do_train']:
        ...
        global_step, tr_loss = train(train_dataset, model, tokenizer,loop,dataset_name,saving_steps,learning_rate)
    ...
    return max(accs.items(), key=operator.itemgetter(1))[0], accs
